...
public class JwtAuthFilter extends AuthenticatingFilter {
    ...
    protected boolean onLoginSuccess(AuthenticationToken token, Subject subject, ServletRequest request, ServletResponse response) throws Exception {
        ...
        TokenBean user = (TokenBean) subject.getPrincipal();
        if (token instanceof JwtToken) {
            JwtToken jwtToken = (JwtToken) token;
            boolean shouldRefresh = shouldTokenRefresh(JwtUtils.getIssuedAt(jwtToken.getToken()));
            System.out.println("刷新标记："+shouldRefresh+":"+JwtUtils.getIssuedAt(jwtToken.getToken()));
            if (shouldRefresh) {
                //生成新token
                newToken = JwtUtils.sign(user.getUsername(), "abc", 3600);
                System.out.println("刷新后token："+newToken);
            }
        }
        if (StringUtils.isNotBlank(newToken)) {
            TokenBean tokenBean = new TokenBean(user.getUsername(),"abc",360);
            redisUtil.set(user.getUsername(), JSON.toJSON(tokenBean), 360);
            System.out.println("存入刷新后的token");
    ...
    protected boolean shouldTokenRefresh(Date issueAt) {
        ...
        System.out.println(LocalDateTime.now().minusSeconds(tokenRefreshInterval));
